...
Without either override, the stable cache path `~/.cache/pi-cliff/cliffcompaction/src` is used.
The generated provenance records the source selector and upstream commit, never a machine path.
"""
...
def main() -> int:
    ...
    if args.upstream_src is not None:
        upstream_src = Path(args.upstream_src).expanduser().resolve()
        source_label = "--upstream-src"
    elif os.environ.get("CLIFF_UPSTREAM_SRC"):
        upstream_src = Path(os.environ["CLIFF_UPSTREAM_SRC"]).expanduser().resolve()
        source_label = "CLIFF_UPSTREAM_SRC"
    else:
        upstream_src = DEFAULT_UPSTREAM_SRC
        source_label = "stable cache default (~/.cache/pi-cliff/cliffcompaction/src)"

    try:
        upstream = import_upstream(upstream_src)
        document = json.loads(Path(args.inputs).read_text(encoding="utf-8"))
        cases = document["cases"]
        print(f"generating {len(cases)} cases from upstream at {upstream_src}")
    ...
    output = {
        ...
        "upstream": {
            ...
            "cloneRev": upstream_revision(upstream_src),
        },
        "defaults": {

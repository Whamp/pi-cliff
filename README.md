...
The five `*MaxTokens` settings are estimates, not actual tokenizer counts: estimated tokens = Unicode code points ÷ 4. They accept finite nonnegative numbers in exact quarter-token steps only, and the converted code-point cap (`tokens * 4`) must be a safe integer. There is no silent rounding. A limit of `0` keeps no content in its category; `"unlimited"` disables it. Positive text limits append `...` after the capped payload. Speaker labels, tool signature wrappers, and appended ellipses are outside text payload caps. Tool-call limits apply to serialized arguments only; tool results are never truncated and oversized results are dropped whole. These are per-content limits, not total-context budgets.

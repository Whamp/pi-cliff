...
- `manual` and `threshold` use the configured renderer. `overflow` disables reasoning and caps `assistantTextMaxChars` at 300, treating `"unlimited"` as 300 and preserving zero. There is no token estimator, fit budget, action-part eviction, or replacement budget knob.
...
Config has seven settings: `mode`, `includeReasoning`, `assistantTextMaxChars`, `reasoningTextMaxChars`, `toolCallMaxChars`, `toolResultMaxChars`, and `userTextMaxChars`. The five character limits accept only nonnegative safe integers or exact `"unlimited"`. Zero retains no category content, including no tool-call line or user/system label; positive limits count Unicode code points. Tool-call limits apply to serialized arguments, not the `[toolName]` wrapper. A tool result above a positive limit is dropped whole. Defaults, `~/.pi/agent/cliff.json`, then `<cwd>/.pi/cliff.json` set precedence. Retired names are rejected with replacement guidance; old text-limit zero values must migrate to `"unlimited"`, while old result-limit zero remains zero.
